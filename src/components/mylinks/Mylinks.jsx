...
function Mylinks({ profile, loading, error, setIsUser, setShowLogin }) {
	...
	const API_BASE_URL =
		import.meta.env.VITE_API_BASE_URL || "https://linkite.onrender.com";
	const API_BASE_SHOW_URL = import.meta.VITE_API_BASE_SHOW_URL || "linkite.onrender.com";
	...
	const copyURL = async (shortUrl) => {
		try {
			await navigator.clipboard.writeText(`${API_BASE_SHOW_URL}/${shortUrl}`);
									...
									<span className="link">
										...
										>
											<i
												...
												style={{ color: "#4f6d64" }}
											></i>{" "}
											{`${API_BASE_SHOW_URL}/${link.shortUrl}`}
										</a>
									</span>
									<i
